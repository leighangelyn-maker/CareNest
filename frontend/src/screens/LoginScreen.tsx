...
import { Field, Sub, inputStyle, Btn } from '../components/atoms';
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (
        ...
        setError('Incorrect email address or password. Please check your details and try again.');
      } else {
        setError(raw.length > 0 && raw.length < 200 ? raw : 'Login failed. Please try again.');
      }
      triggerShake();
    } finally {
      setLoading(false);
          ...
          >
            ...
            {unverifiedEmail ? (
              <View style={styles.resendRow}>
                ...
                {resendMsg ? (
                  <Text style={[styles.resendMsg, { color: resendMsg.includes('resent') ? Colors.success : Colors.danger }]}>
                    {resendMsg}
                  </Text>
                ) : null}
              </View>
            ...
            <Btn onPress={handleLogin} style={loading ? { opacity: 0.6 } : undefined}>
...
const styles = StyleSheet.create({
  ...
  errorBox:     { backgroundColor: Colors.dangerBg, borderWidth: 1, borderColor: Colors.dangerBorder, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 12 },
  errorText:    { fontFamily: Fonts.inter, fontSize: 13, color: Colors.danger, lineHeight: 19 },
  resendRow:    { marginBottom: 12, gap: 6 },
  resendHint:   { fontFamily: Fonts.inter, fontSize: 12.5, color: Colors.slate, lineHeight: 18, marginBottom: 4 },
  ...
  resendMsg:    { fontFamily: Fonts.inter, fontSize: 12.5, lineHeight: 18 },
  hint:         { textAlign: 'center', marginTop: 20, fontSize: 13, color: Colors.slate, fontFamily: Fonts.inter },
